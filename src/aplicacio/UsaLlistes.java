...
public class UsaLlistes {
	...
	public static void main(String[] args) throws FileNotFoundException {
		int opcio, any;
		String nomCientific;
		LlistaPlantacions llistaPlantacions = null;
		LlistaTerrenys llistaTerrenys = null;
		LlistaPlantes llistaPlantes = null;
		do {
		...
		do {
			...
			case 6:
				...
				break;
			case 7:
				System.out.println("Introdueix el nombre científic:");
				nomCientific = teclat.nextLine();
				System.out.println("Introdueix l'edat de la planta:");
				any = teclat.nextInt();
				opcio7(llistaPlantes, nomCientific, any);
				break;
			case 8:
				int tipusPlanta=-1;
				String nom;
				while ((tipusPlanta!=0) || (tipusPlanta!=1)) {
					System.out.println("Introdueix si la nova espècie es Arbústica o Arboreà(Introdueix 0 per Arbústica i 1 per Arboreà:");
					tipusPlanta=teclat.nextInt();
				}
				System.out.println("Introdueix el nom de la nova espècie:");
				nom=teclat.nextLine();
				opcio8(llistaPlantes, nom, tipusPlanta);
				break;
			case 9:
	...
	public static LlistaPlantacions carregaLlistaPlantacions(File aux) throws FileNotFoundException {
		...
		while (sc.hasNextLine()) {
			...
			if (llistaParametres.length % 2 == 0) {
				...
				llista.afegir(
						new Plantacions(llistaParametres[0], Integer.parseInt(llistaParametres[1]), llistaRodals));
			} else {
				sc.close();
				return null;
			}
	...
	public static void mostraMenu() {
		...
		System.out.println(
				"\t14. Mostrar la quantitat de CO2 que permet absorbir el conjunt dunitats plantades dÃ¢una especie en lany actual");
		System.out.println("\t15. Sortir");
		System.out.print("\n\t\t\tIndica opcio:\n");
	}

	// 1. Carregar les dades dels fitxers
	public static void opcio1(LlistaPlantacions llistaPlantacions, LlistaTerrenys llistaTerrenys,
			LlistaPlantes llistaPlantes) throws FileNotFoundException {
		System.out.println("Escull els fitxers per cada llista:");
	...
	// 4. Llistar les dades de les plantacions que tenen algun rodal dun tipus de
	// terreny
	...
	// 5. Donada una plantacio, mostrar quantes unitats de cada especie shi ha
	// plantat
	...
	public static void opcio6(LlistaPlantes llista) {
		System.out.println("Llistant totes les espècies:");
		System.out.println(llista.toString());
	}

	// 7. Donada una especie i una edat, mostrar la quantitat de CO2 que permet
	// absorbir
	public static void opcio7(LlistaPlantes llista, String nomCientific, int any) {
		if (llista.getPlanta(nomCientific)!=null) {
			if (llista.getPlanta(nomCientific).getTipus()==false) {
				...
				System.out.println("Absorció de l'arbust"+arbust.getNomCientific()+"és:"+arbust.getAbsorcio());
			}else {
				Arbres arbre = (Arbres) llista.getPlanta(nomCientific);
				System.out.println("Absorció de l'arbre"+arbre.getNomCientific()+"és:"+arbre.getAbsorcio());
			}
		}else {
	...
	public static void opcio8(LlistaPlantes llistaPlantes, String nom, int tipusPlanta) {
		int edatMaxima;
		if (tipusPlanta==0) {
			int absorcio;
			System.out.println("Introdueix l'absorció de l'arbust:");
			absorcio=teclat.nextInt();
			System.out.println("Introdeix l'edat màxima:");
			edatMaxima=teclat.nextInt();
			...
			System.out.println("Quants intervals té aquest arbre?");
			n_intervals=teclat.nextInt();
			absorcio = new int[n_intervals];
			intervals = new int[n_intervals];
			for (int i=0; i<n_intervals;i++) {
				System.out.println("Introdueix l'absorció"+i+":");
				absorcio[i]=teclat.nextInt();
				System.out.println("Introdueix l'interval"+i+":");
				intervals[i]=teclat.nextInt();
			}
			llistaPlantes.afegirPlanta(new Arbres(nom, absorcio, intervals));
	...
	public static void opcio11(LlistaPlantacions llista) {
		...
		if (trobat == false) {
			...
			if (llista.getLlista()[i].getRodals().getNumElem() - 1 > j) {
				...
				System.out
						.println("Aquest rodal te: " + llista.getLlista()[i].getRodals().getLlistaRodals(j).toString());
				String opcio = "";
				...
				if (opcio.equalsIgnoreCase("Si")) {
					...
					if (llista.getLlista()[i].getRodals().getLlistaRodals(j).getTipusTerreny()
							.equalsIgnoreCase(terreny)) {
	...
	// 12. Modificar lany en que ens trobem (valor que hem introduit a linici pero
	// que volem
	...
	// 13. Mostrar la quantitat de CO2 que permet absorbir cada rodal d'una plantacio
	// en l'any actual
	...
	// 14. Mostrar la quantitat de CO2 que permet absorbir el conjunt dunitats
	// plantades d'una especie
	// (entre totes les plantacions que tenim guardades) en lÃ¢â‚¬â„¢any actual.
	// Indicarem el nom de
	// lÃ¢â‚¬â„¢especie per teclat.
	public static void opcio14() {
		
	}

	// 15. Sortir. Permetre sortir guardant la informaciÃƒÂ³ de les classes als
	// fitxers o no.
	public static void opcio15(LlistaPlantacions llistaPlantacions, LlistaTerrenys llistaTerrenys,
